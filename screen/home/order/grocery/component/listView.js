...
import { View, Text, ActivityIndicator, Image, FlatList, ScrollView, TouchableOpacity } from 'react-native';
...
function ListView(props) {
    ...
    React.useEffect(() => {
        props.navigation.setOptions({
            ...
            headerRight: () => (
                <TouchableOpacity activeOpacity={0.7} onPress={() => props.navigation.navigate('Search', { name: props.route.params.item.name })} >
                    <MaterialCommunityIcons name='magnify' color={'#4f4f4f'} size={25}
                        style={{
                            marginRight: 10,
                            padding: 6,
    ...
    return (
        !state.isLoading ?
            <View style={styles.container}>
                                ...
                                state.list.map(({ name }, index) =>
                                    ...
                                    { backgroundColor: name === state.selectedProduct.name ? '#e4545f' : '#f9f9f9', marginLeft: index === 0 ? 0 : 10 }]}
                                        ...
                                        <Text numberOfLines={2} style={{
                                            ...
                                            color: state.selectedProduct.name === name ? selectedColor : unSelectedColor
                                        }}>{name}</Text>
                ...
                <FlatList
                    style={{ flex: 1 }}
                    data={Object.keys(state.selectedProduct).length !== 0 ? state.selectedProduct.list : state.list}
                    renderItem={renderItem}
                    keyExtractor={(item, index) => index.toString()}
                    initialNumToRender={5}
                    showsVerticalScrollIndicator={false}
                    key={Object.keys(state.selectedProduct).length !== 0 ? state.selectedProduct.list : state.list}
                />
